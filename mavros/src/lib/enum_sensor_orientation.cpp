...
// std::pair 是标准库提供的一个模板类，用于存储两个可能不同类型的数据作为一个整体
using OrientationPair = std::pair<const std::string, const Eigen::Quaterniond>;

// internal data initializer
// 创建 "方向对"（OrientationPair）的辅助函数，主要功能是将以度为单位的欧拉角（roll, pitch, yaw）转换为四元数，并与名称关联起来。
// 定义传感器或机体的坐标系方向
// Return Example: make_orientation("camera", 0.0, 0.0, 90.0);
static const OrientationPair make_orientation(const std::string &name,
	const double roll,
	const double pitch,
	const double yaw)
{
	// 定义了角度到弧度的转换因子，constexpr表示这是一个编译期常量
	constexpr auto DEG_TO_RAD = (M_PI / 180.0);
	// 首先将 roll, pitch, yaw 打包成 Eigen 向量
	// 乘以转换因子将角度转为弧度
	// 使用ftf::quaternion_from_rpy函数将欧拉角转换为四元数
...
// 用于存储无人机传感器的标准安装朝向配置,MAV_SENSOR_ORIENTATION：是 MAVLink 协议定义的传感器朝向标准枚举.
// std::unordered_map 是 C++ 标准库中的一个容器，属于无序关联容器，用于存储键值对（key-value pairs）----> dict{}
// std::underlying_type 是 C++ 标准库中的一个模板类（定义在 <type_traits> 头文件中），用于获取枚举类型的底层整数类型。
 ...
 * 	通过发送 MAV_SENSOR_ORIENTATION 枚举值获取定传感器的 std::string (user defined)
 */
 ...
 * 
 * 	通过发送 MAV_SENSOR_ORIENTATION 枚举值获取定传感器的quaterniond(user defined)
 */
